...
//     Phase 3: Material variations (reference camera with materials 0-N, default light, no fog)
...
void UDatasetCaptureManager::ProcessCaptureState()
{
    ...
    // Phase 3: Material variations (with reference camera, default light, no fog)
...
void UDatasetCaptureManager::SpawnCurrentMesh()
{
    ...
    // Ensure the actor is exactly at the target point location (with center alignment)
    ...
    // Calculate offset to move actor center to target point center
    const FVector TargetLocation = SpawnXf.GetLocation();
    const FVector OffsetToTarget = TargetLocation - BoundsCenter;
...
void UDatasetCaptureManager::NormalizeActorScale()
{
    ...
    const FBox Bounds = m_pCurrentSpawnedActor->GetComponentsBoundingBox(true);
    const float CurrentRadius = Bounds.GetExtent().Size();

    if (CurrentRadius < KINDA_SMALL_NUMBER)
    {
        UE_LOG(LogTemp, Warning, TEXT("Capture Manager: NormalizeActorScale() - mesh radius too small: %.2f"), CurrentRadius);
        return;
    }

    const float ScaleFactor = m_fTargetRadius / CurrentRadius;
    m_pCurrentSpawnedActor->SetActorScale3D(FVector(ScaleFactor));

    UE_LOG(LogTemp, Log, TEXT("Capture Manager: NormalizeActorScale() - Target: %.2f cm, Current: %.2f cm, ScaleFactor: %.4f"), 
        m_fTargetRadius, CurrentRadius, ScaleFactor);
}
    ...
    else
    {
        ...
        switch (m_eCapturePhase)
        {
            ...
            case ECapturePhase::MaterialVariations:
                ...
                if (m_aMaterials.IsValidIndex(m_iCurrentMaterialIndex))
                {
                    VariationFolder = m_aMaterials[m_iCurrentMaterialIndex]->GetName();
                }
                else
                {
                    VariationFolder = TEXT("DefaultMaterial");
                }
                break;
...
void UDatasetCaptureManager::CaptureSegmentationMask(int32 ScreenshotWidth, int32 ScreenshotHeight)
{
    ...
    // Hide fog during mask capture — volumetric effects bypass ShowOnlyActors
...
void UDatasetCaptureManager::BuildCameraTargetsForCurrentActor()
{
    ...
    // Use fixed camera distance - doesn't adjust based on model size
    // This allows the radius parameter to control apparent model size on screen
    // Adjust this value to control model size relative to screen:
    // - Smaller value (e.g., 80cm) = larger model on screen
    // - Larger value (e.g., 150cm) = smaller model on screen
    // For FOV=50° at 100cm: a 50cm model takes up ~50% of screen
    constexpr float FixedCamDistance = 100.f; // Fixed distance in cm from object center
    ...
    for (const FVector& Dir : m_vCameraTransforms)
    {
        ...
        m_aCameraTargets.Add(m_vCurrentObjectCenter + WorldDir * FixedCamDistance);
    }
